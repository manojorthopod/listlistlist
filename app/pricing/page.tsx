...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: 'Before any listing is generated you see every extracted field — product type, material, colour, condition, features — and can edit them. Nothing generates until you confirm. Garbage in, garbage out; accurate confirmation screen = accurate listings.',
  },
  {
    q: 'Do credits expire?',
    ...
    <div className="min-h-screen bg-base">
      ...
      <nav className="border-b border-border">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          <Link href="/" className="font-mono font-bold text-lg text-text-primary">
            listlistlist
          </Link>
          <div className="flex items-center gap-4">
            {isSignedIn ? (
              <Link
                href="/dashboard"
                className="border border-border-2 hover:border-accent text-text-primary font-medium rounded-lg px-4 py-2 text-sm transition-colors duration-150"
              >
                Dashboard
              </Link>
            ) : (
              <>
                ...
                <Link
                  href="/sign-up"
                  className="bg-accent hover:bg-accent-hover text-white font-semibold rounded-lg px-4 py-2 text-sm transition-colors duration-150"
                >
                  Start free trial
                </Link>
              </>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="bg-surface border border-border rounded-xl p-8 mb-20">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
            {[
              { name: 'Amazon',       color: 'text-platform-amazon' },
              { name: 'Etsy',         color: 'text-platform-etsy' },
              { name: 'eBay',         color: 'text-platform-ebay' },
              { name: 'Shopify',      color: 'text-platform-shopify' },
              { name: 'WooCommerce',  color: 'text-platform-woo' },
              { name: 'TikTok Shop',  color: 'text-platform-tiktok' },
            ].map((p) => (
              <div
                key={p.name}
            ...
            {FAQ_ITEMS.map((item, i) => (
              ...
              >
                ...
                >
                  ...
                  <span className={`text-text-secondary shrink-0 transition-transform duration-150 ${openFaq === i ? 'rotate-45' : ''}`}>
                    +
                  </span>
                </button>
                {openFaq === i && (
                  <div className="px-6 pb-5 text-text-secondary text-sm leading-relaxed border-t border-border">
        ...
        {!isSignedIn && (
          <div className="text-center bg-surface border border-border rounded-2xl p-12">
            ...
            <Link
              href="/sign-up"
              className="inline-block mt-6 bg-accent hover:bg-accent-hover text-white font-semibold rounded-lg px-6 py-3 transition-colors duration-150"
            >
